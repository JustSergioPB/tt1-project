...
void vmpcm(int rows, int columns, double *tau, double ***x_guess, double omega1, double omega2, double errorTolerance, double varargin){
    ...
    for(int i = 0; i < 300 && (any(rows, a) == 1 || any(rows, b) == 1); i++){
        ...
        for(int j = 0; j < columns; j++){
            ...
            beta_k[0][j] = dot + 2*x_guessAux[0][j];
        }
        ...
        for(int k = 0; k < rows; k++){
            err2[k] = err1[k];
            err1[k] = fabs(x_new[k][0] - x_guessAux[k][0]);
            for(int j = 0; j < columns; j++ ){
                if(err1[k] < fabs(x_new[k][j] - x_guessAux[k][j]))
                    err1[k] = fabs(x_new[k][j] - x_guessAux[k][j]);

                x_guessAux[k][j] = x_new[k][j];
            }
        }
...
{
    double **m = (double **) calloc(sizeK, sizeof(double *));

    for(int i = 0; i < sizeK; i++){
        m[i] = (double *) calloc(sizeTau, sizeof(double ));
        for(int j = 0; j < sizeTau; j++){
            m[i][j] = cos( k[i] * creal(acos(tau[j])));
        }
    }
